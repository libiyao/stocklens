...
export function calculateTechnicalScores(candles: Candle[], levels: PriceLevel[]): TechnicalScores {
  ...
  const trendSignals: Array<[number | null | undefined, number]> = [[sma20, 14], [sma50, 13], [sma200, 13]];
  const trend = Math.round(trendSignals.reduce((score, [average, weight]) => {
    if (average == null) return score;
    return score + (current > average ? weight : -weight);
  }, 50));
...
export function generateTheRead(candles: Candle[], scores: TechnicalScores, levels: PriceLevel[]): string {
  ...
  const support = levels.filter(l => l.type === "support" && l.price < close).sort((a, b) => b.price - a.price)[0];
  const resistance = levels.filter(l => l.type === "resistance" && l.price > close).sort((a, b) => a.price - b.price)[0];
